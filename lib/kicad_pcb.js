...
function pcb2lines(kicad_pcb) {
  ...
  return lines.map(function (l) {
    ...
    var footprint_name = footprint.split(':')[1].replace(/_/g, ' ');
    value = value.replace(/_/g, ' ');
    var description = value + ' ' + footprint_name;
    var component = electroGrammar.parse(description, { returnIgnored: true });
    if (component.size) {
      var egValue = component.resistance || component.capacitance;
      // ignore if no value, or when the value actually looks like a footprint,
      // or if we got no info from the footprint name
      if (egValue && !RegExp(egValue).test(footprint_name) && component.ignored !== footprint_name) {
        return { reference: reference, description: value + ' ' + component.size };
      }
    }
