let {retailer_list} = require('./line_data');
...
exports.writeTSV = function(lines) {
    ...
    let r = 'References\tQty\tDescription';
    ...
    let maxParts = Math.max.apply(null, partsLengths);

    if (maxParts >= 1) {
        for (let _ of __range__(1, maxParts, true)) {
            r += '\tManufacturer';
            r += '\tMPN';
        }
    }
    ...
    for (line of lines) {
        ...
        r += `\t${line.description}`;
        if (maxParts >= 1) {
            for (let i of __range__(0, (maxParts - 1), true)) {
                r += '\t';
